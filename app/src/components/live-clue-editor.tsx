...
export function LiveClueEditor({
  ...
  // Track which words we've already auto-generated for
  const autoGeneratedWords = useRef<Set<string>>(new Set());
  ...
  // Auto-generate clues for words that don't have any
  useEffect(() => {
    for (const wordInfo of detectedWords) {
      const wordClues = clues[wordInfo.word];
      const hasAnyClue = wordClues?.easy || wordClues?.medium || wordClues?.hard;
      const alreadyGenerated = autoGeneratedWords.current.has(wordInfo.word);
      const isLoading = loadingAI === wordInfo.word;

      if (!hasAnyClue && !alreadyGenerated && !isLoading && onClueOptionsUpdate) {
        autoGeneratedWords.current.add(wordInfo.word);
        handleAskAI(wordInfo.word);
      }
    }
  }, [detectedWords, clues, loadingAI, handleAskAI, onClueOptionsUpdate]);
